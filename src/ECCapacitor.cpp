...
#define CAPACITOR_RADIUS 0.006
#define CAPACITOR_HEIGHT 0.002

const Interface ECCapacitor::interface(
	"Capacitor", typeid( ECCapacitor ),
	Create<ECCapacitor>::create,
	&ECLine::interface
);
...
ECCapacitor::ECCapacitor()
{
	...
	cyl->radius->set(CAPACITOR_RADIUS);
	cyl->height->set(CAPACITOR_HEIGHT);
	cyl2->radius->set(CAPACITOR_RADIUS);
	cyl2->height->set(CAPACITOR_HEIGHT);
	...
	pos->translation->set(Vec3f(0, CAPACITOR_HEIGHT, 0));
	pos2->translation->set(Vec3f(0, -CAPACITOR_HEIGHT, 0));
...
ECCapacitor::ECCapacitor(const ECCapacitor &)
{
}

void ECCapacitor::subSetActif(bool a)
{
	if (a)
	{
...
void ECCapacitor::initialize()
{
}

string ECCapacitor::getEffectMessage() const
{
